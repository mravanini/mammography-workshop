...
Our team is done, we can select it in the labeling job we are creating in the next step.

![Image](./images/best_doctors_done.png)

Now we have a workforce, we need to upload the images we want to classify to Amazon S3 and create a labeling job assigned to our private team.

Each member of the list will receive an e-mail like this:
...
This is where we create the labeling job. Ground Truth expects your data to be in S3, so let's upload them.

### Part 1: Uploading the images

Prepare your pictures to upload:

![Image](./images/explorer.png)

Open the **Amazon S3 Console** and create a new **bucket**:

![Image](./images/s3_console.png)
...
In my case, it is **s3://mammography-images-for-labeling/**.
...
Define a name for your job, something very cool like **mammography-views**. A labeling job requires a **manifest.json** file containing the location of your images. This file can be automatically generated by AWS, we just need to select the S3 folder where we uploaded the mammography files in the previous part. Click on **Create manifest file**:
